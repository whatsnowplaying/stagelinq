...
from stagelinq.messages import (
    DiscoveryMessage, StateSubscribeMessage, StateEmitMessage,
    DISCOVERER_HOWDY, Token
)


class TestRealSC6000Reference:
    """Test suite using real SC6000 data as the authoritative reference."""
    ...
    def setup_class(cls):
        ...
        with open(packets_file, 'r') as f:
            cls.packet_data = json.load(f)
    
    def test_discovery_packet_parsing(self):
        """Test parsing of real SC6000 discovery packets."""
        # Use the first discovery packet as reference
        packet_hex = self.packet_data["discovery_packets"][0]
    ...
    def test_discovery_packet_consistency(self):
        """Test that all discovery packets are consistent."""
        packets = self.packet_data["discovery_packets"]
    ...
    def test_state_subscribe_packet_parsing(self):
        ...
        for packet_hex in self.packet_data["state_packets"]:
            packet_bytes = binascii.unhexlify(packet_hex)
            # Check magic ID
            import struct
        ...
        assert len(state_subscribe_packets) > 0, "No state subscribe packets found"
    ...
    def test_state_emit_packet_parsing(self):
        ...
        assert len(state_emit_packets) > 0, "No state emit packets found"
    ...
    def test_discovery_roundtrip_serialization(self):
    ...
    def test_packet_capture_metadata(self):
        """Test that packet capture metadata is correctly understood."""
        summary = self.packet_data["summary"]
        ...
        assert len(self.packet_data["discovery_packets"]) <= summary["total_discovery"]
        assert len(self.packet_data["state_packets"]) <= summary["total_state"]
        assert len(self.packet_data["beat_packets"]) == summary["total_beat"]
    
    def test_state_packet_types(self):
        ...
        assert state_subscribe_count + state_emit_count == len(self.packet_data["state_packets"])
    
    def test_utf16_string_encoding(self):
    ...
    def test_all_discovery_packets_parseable(self, packet_index):
        """Test that all discovery packets in the capture are parseable."""
        packet_hex = self.packet_data["discovery_packets"][packet_index]
        packet_bytes = binascii.unhexlify(packet_hex)
    ...
    def test_real_world_token_format(self):
        ...
        for packet_hex in self.packet_data["discovery_packets"]:
            packet_bytes = binascii.unhexlify(packet_hex)
