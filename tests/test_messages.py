...
def test_state_emit_message():
    ...
    msg = StateEmitMessage(
        name="/Engine/Deck1/CurrentBPM",
        json_data='{"value": 128.5}'
    )
...
def test_beat_emit_message_validation():
    ...
    writer = io.BytesIO()
    with pytest.raises(ValueError, match="Number of players must match number of timelines"):
        msg.write_to(writer)
...
@pytest.mark.parametrize("message_id,expected_class", [
...
def test_message_id_constants(message_id, expected_class):
    ...
    assert expected_class.MESSAGE_ID == message_id
