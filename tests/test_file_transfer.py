...
class TestFileInfo:
    ...
    def test_directory_info_creation(self):
        ...
        dir_info = FileInfo(path="/Music", name="Music", is_directory=True)
...
class TestFileSource:
    ...
    def test_file_source_creation(self):
        ...
        source = FileSource(name="Music (SD)", database_path="/Music/database.db")
...
class TestFileTransferConnection:
    ...
    async def test_connect_success(self):
        ...
        with patch(
            "stagelinq.file_transfer.StageLinqConnection",
            return_value=mock_stagelinq_conn,
        ):
            await conn.connect()
    ...
    async def test_connect_failure(self):
        ...
        with patch(
            ...
            with pytest.raises(
                ConnectionError, match="Failed to connect to FileTransfer service"
            ):
                await conn.connect()
    ...
    async def test_download_database_source_not_found(self):
        ...
        mock_stagelinq_conn = AsyncMock()
        conn._connection = mock_stagelinq_conn
        conn._sources = [FileSource("Other Source", "/path")]
    ...
    async def test_get_database_info_success(self):
        ...
        test_source = FileSource("Music (SD)", "/Music/database.db")
    ...
    async def test_list_sources(self):
        ...
        test_sources = [
            FileSource("Music (SD)", "/Music/database.db"),
            FileSource("Music (USB)", "/USB/database.db"),
        ]
