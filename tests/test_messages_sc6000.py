...
def test_token_creation_16_bytes():
...
def test_token_from_sc6000_data(sc6000_packet_data):
...
def test_token_string_representation(sc6000_packet_data):
...
def test_discovery_message_parsing_real_data(sc6000_packet_data):
...
def test_discovery_message_roundtrip_sc6000(sc6000_packet_data):
...
def test_discovery_message_all_packets_consistent(sc6000_packet_data):
...
def test_discovery_message_magic_bytes(sc6000_packet_data):
    """Test that discovery messages have correct magic bytes."""
    packet_hex = sc6000_packet_data["discovery_packets"][0]
    packet_bytes = binascii.unhexlify(packet_hex)

    # Should start with "airD"
    assert packet_bytes[:4] == DISCOVERY_MAGIC

    # Parse and verify magic is preserved
    message = DiscoveryMessage.deserialize(packet_bytes)
    serialized = message.serialize()
    assert serialized[:4] == DISCOVERY_MAGIC


def test_discovery_message_utf16_encoding(sc6000_packet_data):
...
def test_state_subscribe_message_parsing(sc6000_packet_data):
...
def test_state_emit_message_parsing(sc6000_packet_data):
...
def test_state_message_smaa_magic(sc6000_packet_data):
...
def test_discovery_magic_constant(sc6000_packet_data):
...
def test_smaa_magic_constant(sc6000_packet_data):
...
def test_discoverer_howdy_constant(sc6000_packet_data):
...
def test_packet_capture_metadata(sc6000_packet_data):
...
def test_real_device_characteristics(sc6000_packet_data):
...
def test_all_packets_parseable(sc6000_packet_data):
...
def test_discovery_packet_consistency(sc6000_packet_data, packet_index):
