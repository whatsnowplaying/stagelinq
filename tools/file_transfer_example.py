    ...
    async with discover_stagelinq_devices(config) as discovery:
        ...
        try:
            ...
            async with connection:
                ...
                file_transfer_service = next(
                    (service for service in services if service.name == "FileTransfer"),
                    None,
                )
                ...
                print(
                    f"OK FileTransfer service found on port {file_transfer_service.port}"
                )
                        ...
                        try:
                            ...
                            if sources:
                                ...
                                for source in sources:
                                    ...
                                    try:
                                        db_info = await file_transfer.get_database_info(
                                            source
                                        )
                                        print(
                                            f"     Database: {db_info['database_path']}"
                                        )
                                        print(
                                            f"     Size: {db_info['database_size']} bytes"
                                        )
