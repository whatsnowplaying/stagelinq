...
def extract_tcp_packets(pcap_file: Path, port_filter: str = "") -> list[dict[str, Any]]:
    ...
    filter_expr = f"tcp and ({port_filter})" if port_filter else "tcp"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Extract StageLinq packets using tshark"
    )
    ...
    if discovery_packets:
        ...
        for i, packet in enumerate(discovery_packets[:3]):
            print(
                f"  {i + 1}. {packet['src_ip']} -> {packet['dst_ip']} (port {packet['dst_port']})"
            )
    ...
    if args.output:
        ...
        print("\nTest data preview:")
        print(f"{json.dumps(test_data, indent=2)[:500]}...")
